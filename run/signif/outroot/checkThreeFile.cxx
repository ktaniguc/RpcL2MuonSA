...
//  TFile *_file0 = TFile::Open("rpcCluster_JpsiCloseBy_renewAlgo_202007171628.root");
  TFile *_file0 = TFile::Open("separateMDTs/rpcCluster_JpsiCloseBy_renewAlgo.root");
//  TFile *_file1 = TFile::Open("changerWidth/rpcCluster_JpsiCloseBy_renewAlgo.root");
//  TFile *_file2 = TFile::Open("changerWidth_150_120_150/rpcCluster_JpsiCloseBy_renewAlgo.root");
//  TFile *_file0 = TFile::Open("residualWeight_norm30_m0p3/rpcCluster_JpsiCloseBy_renewAlgo.root");
//  TFile *_file1 = TFile::Open("residualWeight_norm30_m0p57/rpcCluster_JpsiCloseBy_renewAlgo.root");
//  TFile *_file2 = TFile::Open("residualWeight_norm30_m0p71/rpcCluster_JpsiCloseBy_renewAlgo.root");
  TFile *_file1 = TFile::Open("residualWeight_norm16_m0p4/rpcCluster_JpsiCloseBy_renewAlgo.root");
  TFile *_file2 = TFile::Open("residualWeight_norm16_m0p4_v3/rpcCluster_JpsiCloseBy_renewAlgo.root");
//  TFile *_file1 = TFile::Open("rpcCluster_JpsiCloseBy_oldL2SA_202007201337.root");
  if(checkPtres){
    ...
    TH1D *h2 = (TH1D*)_file2->Get(histname[0].c_str());
//    TH1D *h3 = (TH1D*)_file3->Get(histname[0].c_str());
    //  h0->Sumw2();
...
//    h3->Draw("hist same");
    ...
    TLegend_addfunc *legptres = new TLegend_addfunc(0, 0, 7);
    legptres->AddSelection("J/#psi->#mu#mu");
    legptres->AddSelection("2mu-in-1RoI");
    legptres->AddSelection("muon #eta_{MS} < 1.05");
    legptres->AddEntry(h0, "separateMDTs");
//    legptres->AddEntry(h1, "rWidth 90,60,90");
//    legptres->AddEntry(h2, "rWidth 150,120,150");
//    legptres->AddEntry(h0, "weight A=30 B=-0.3");
    legptres->AddEntry(h1, "weight A=16 B=-0.4 wofix");
    legptres->AddEntry(h2, "weight A=16 B=-0.4 fixed");
//    legptres->AddEntry(h2, "weight A=30 B=-0.71");
//    legptres->AddEntry(h3, "weight A=16 B=-0.4");
    legptres->Draw("same");
  ...
  if(checkEfficiency){
    ...
    if(h_den_newL2SA != nullptr){
      ...
      leg->AddEntry(m_peff_2pt_newL2SA, "changerWidth");
